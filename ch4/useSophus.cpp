...
int main(int argc, char **argv) {
  ...
  // Rotation matrix that rotates 90 degrees along the Z axis
  Matrix3d ​​R = AngleAxisd(M_PI / 2, Vector3d(0, 0, 1)).toRotationMatrix();
  // or quaternion
  Quaterniond q(R);
  Sophus::SO3d SO3_R(R); // Sophus::SO3d can be constructed directly from the rotation matrix
  Sophus::SO3d SO3_q(q); // can also be constructed from quaternions
  // both are equivalent
  ...
  // Conversely, vee is antisymmetric to a vector
  cout << "so3 hat vee= " << Sophus::SO3d::vee(Sophus::SO3d::hat(so3)).transpose() << endl;

  // update of incremental perturbation model
  Vector3d update_so3(1e-4, 0, 0); //Assume the update amount is so much
  Sophus::SO3d SO3_updated = Sophus::SO3d::exp(update_so3) * SO3_R;
  cout << "SO3 updated = \n" << SO3_updated.matrix() << endl;

  cout << "********************************" << endl;
  // Similar to SE(3) operation
  ...
  cout << "SE3 from R,t= \n" << SE3_Rt.matrix() << endl;
  cout << "SE3 from q,t= \n" << SE3_qt.matrix() << endl;
  // Lie algebra se(3) is a six-dimensional vector, typedef first for convenience
